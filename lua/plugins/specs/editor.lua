...
return {
    ...
    },
    {
        ...
        keys = {
            { "<leader>mo", "<cmd>OverseerToggle<cr>", desc = "构建输出" },
        },
        opts = {
            task_list = {
                direction = "bottom",
    ...
    {
        -- 自动补全括号、引号等（mini 生态，比 nvim-autopairs 更轻量）。
    ...
    {
        -- Treesitter 语法树支持（新 API：仅负责 parser 安装，highlight 由 Neovim 内置处理）。
        "nvim-treesitter/nvim-treesitter",
        build = ":TSInstall bash c css cmake cpp dockerfile git_config gitcommit gitignore go html javascript json lua make markdown markdown_inline python query rust sql toml tsx typescript vim vimdoc xml yaml",
        event = { "BufReadPost", "BufNewFile" },
        config = function()
            -- 新 API 的 setup() 只接受 install_dir，其余选项均已移除。
            ...
            -- 对所有已有 parser 的文件类型启用内置 highlight。
            vim.api.nvim_create_autocmd("FileType", {
                group = vim.api.nvim_create_augroup("user_treesitter_hl", { clear = true }),
                callback = function()
